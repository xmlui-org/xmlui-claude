...
$installDir = if ($env:CLAUDE_PLUGIN_DATA) {
  Join-Path $env:CLAUDE_PLUGIN_DATA "bin"
} elseif ($env:XMLUI_CLI_INSTALL_DIR) {
  $env:XMLUI_CLI_INSTALL_DIR
} else {
  Join-Path $HOME "bin"
}
...
try {
  ...
  if ($env:CLAUDE_PLUGIN_DATA) {
    Write-Log "Binary managed by the plugin at $targetPath"
  } else {
    ...
    if ($pathSegments -notcontains $installDir) {
      ...
      Write-Log "Added $installDir to user PATH (takes effect in new terminals)"
    } else {
